...
const BuyProperties = () => {
  ...
  // Default to 'SELL' if no type is passed via location.state
  const propertyType = location.state?.type || 'SELL';
  const navigate = useNavigate();

  useEffect(() => {
    const fetchProperties = async () => {
      try {
        // IMPORTANT: Changed API endpoint to fetch owner information
        // Ensure your backend provides owner data for this endpoint:
        const response = await axios.get(
          `http://localhost:8080/api/property/type-with-owner/${propertyType}`, // Changed from /type/ to /type-with-owner/
    ...
    fetchProperties();
  }, [propertyType]); // Depend on propertyType to refetch if it changes
    ...
    <div style={{ display: 'flex', justifyContent: 'center', marginTop: '80px' }}>
      ...
      >
        ...
        >
          ...
          />
          Available {propertyType === 'SELL' ? 'Buy' : propertyType} Properties
          <img
            src="https://img.icons8.com/?size=100&id=46774&format=png&color=000000"
            alt="End Icon"
            style={{ width: '35px', height: '35px' }}
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', fontWeight: 600, color: '#666' }}>
            No {propertyType === 'SELL' ? 'buy' : propertyType.toLowerCase()} properties found.
          </div>
        ) : (
            ...
            {properties.map((property) => (
              ...
              >
                ...
                <div style={{ padding: '16px', boxSizing: 'content-box' }}>
                  ...
                  {/* 👇 ATTRACTIVE OWNER INFO SECTION (Copied from RentProperties) */}
                  {property.user && (
                    <div
                    style={{
                      marginTop: '15px',
